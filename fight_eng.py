...
def main():
    ...
    if pg.mixer.get_busy():
        pg.mixer.stop()
    pg.mixer.music.load("audio\\Something is near.mp3")
    pg.mixer.music.play()
    ...
    def win():
        ...
        layer.set_alpha(10)
        font = pg.font.SysFont(None, 160)
        win_text = font.render("V I C T O R Y", True, (0, 0, 120))
        
        screen.blit(layer, (0, 0))
        screen.blit(win_text, win_text.get_rect(center = (scr_w / 2, scr_h / 2)))
        button_font = pg.font.Font(chi_font, 18)
        main_menu = pg.draw.rect(screen, (140, 80, 0), (100, scr_h / 2 + 80, 200, 50))
        main_menu_text = button_font.render("Back to Main Title", True, (255, 255, 255))
        next_level = pg.draw.rect(screen, (140, 80, 0), (500, scr_h / 2 + 80, 200, 50))
        next_level_text = button_font.render("Go to the next level", True, (255, 255, 255))
    ...
    def lose():
        ...
        layer.set_alpha(10)
        font = pg.font.SysFont(None, 160)
        lose_text = font.render("L   O   S   E", True, (255, 0, 0))
    ...
    # 設定背景圖片
    try:
        bg = pg.image.load("img\\fight_bg.png").convert()
        bg_rect = bg.get_rect(center = (scr_w / 2, scr_h / 2))
        screen.blit(bg, bg_rect)
    except Exception as e:
        messagebox.showerror("Game Error", f"Failed to load background images: {e}")
    
    # 設定敵人圖片
    try:
        enemy = pg.image.load("img\\enemy1_tpbg.png").convert_alpha()
        enemy = pg.transform.scale(enemy, (scr_h - 250, scr_h - 250))
        enemy_rect = enemy.get_rect(center = (scr_w / 2, 250))
        screen.blit(enemy, enemy_rect)
    except Exception as e:
        messagebox.showerror("Game Error", f"Failed to load enemy image: {e}")
    ...
    try:
        ...
        messagebox.showerror("Game Error", f"Failed to load character data: {e}")
    ...
    for i in char_select:
        for j in data:
            if int(i) == j["Num"]:
                char_name.append(j["Name"])
                char_hp.append(j["HP"])
                char_attack_note.append(j["Attack_note_eng"])
    ...
    try:
        ...
        messagebox.showerror("Game Error", f"Failed to load character images: {e}")
                                
    # 設定角色名字
    char_font = pg.font.Font(chi_font, 24)
    char_margin = 65
    ...
    # 設定戰鬥狀態區塊
    rect_text = pg.draw.rect(screen, (80, 80, 80), (600, scr_h - fight_h, 200, fight_h))
    status_font = pg.font.Font(chi_font, 13)
    status_line_lim = 9
    status_lst = []
    status_text = "Attack in the remaining rounds."
    status_update(status_text)
    status_text = "Click on the character to attack."
    status_update(status_text)

    # 設定說明區塊
    note = "Right-click on the character to check the attack style."
    ...
    # 設定回合數
    round_count = 10
    round_font = pg.font.SysFont(None, 20)
    def round():
        if round_count <= 2:
            round_text = round_font.render("Remaining Rounds: " + str(round_count), True, (255, 0, 0))
        else:
            round_text = round_font.render("Remaining Rounds: " + str(round_count), True, (255, 255, 255))
        ...
        for event in pg.event.get():
            ...
            if battle and event.type == pg.MOUSEBUTTONUP and left_click:
                if player and rect_char1.collidepoint(pos):
                    ...
                    if bt_hp[0] == 0:
                        atk = 0
                        status_text = bt_name[0] + " has already fainted."
                    else:
                        if crit_status == True:
                            status_text = bt_name[0] + " made a " + str(atk) + " critical hit."
                        else:
                            status_text = bt_name[0] + " dealt " + str(atk) + " damage."
                        player = False
                    status_update(status_text)
                elif player and rect_char2.collidepoint(pos):
                    ...
                    if bt_hp[1] == 0:
                        atk = 0
                        status_text = bt_name[1] + " has already fainted."
                    else:
                        if crit_status == True:
                            status_text = bt_name[1] + " made a " + str(atk) + " critical hit."
                        else:
                            status_text = bt_name[1] + " dealt " + str(atk) + " damage."
                        player = False
                    status_update(status_text)
                elif player and rect_char3.collidepoint(pos):
                    ...
                    if bt_hp[2] == 0:
                        atk = 0
                        status_text = bt_name[2] + " has already fainted."
                    else:
                        if crit_status == True:
                            status_text = bt_name[2] + " made a " + str(atk) + " critical hit."
                        else:
                            status_text = bt_name[2] + " dealt " + str(atk) + " damage."
                        player = False
                    status_update(status_text)
                elif player and rect_char4.collidepoint(pos):
                    ...
                    if bt_hp[3] == 0:
                        atk = 0
                        status_text = bt_name[3] + " has already fainted."
                    else:
                        if crit_status == True:
                            status_text = bt_name[3] + " made a " + str(atk) + " critical hit."
                        else:
                            status_text = bt_name[3] + " dealt " + str(atk) + " damage."   
                        player = False
                    status_update(status_text)
                else:
                    break
                ...
                if player == False and enemy_hp > 0:
                    ...
                    while enemy_atk:
                        ...
                        if bt_hp[i] == 0:
                            ...
                            status_text = bt_name[i] + " was hurt by a " + str(enemy_atk) + " hit."
                            status_update(status_text)
                    ...
                    if target - enemy_atk <= 0 and target != 0:
                        bt_hp[i] = 0
                        status_text = bt_name[i] + " is unable to fight."
                        status_update(status_text)
